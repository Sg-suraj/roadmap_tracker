document.addEventListener('DOMContentLoaded', () => {
    ...
    if (page === 'dashboard') {
        ...
        weeksGrid?.addEventListener('click', async (e) => {
            ...
            const weekNumber = weekCard.dataset.weekNumber ?? targetButton.dataset.weekNumber;
            if (!weekNumber) return;

            // DELETE
            if (targetButton.classList.contains('delete-week-btn')) {
                if (!confirm(`Are you sure you want to delete Week ${weekNumber} and all its tasks? This cannot be undone.`)) return;
            ...
            if (targetButton.classList.contains('edit-week-btn')) {
                try {
                    ...
                    alert('Failed to load week data for editing.');
                }
            }
        });
        ...
        async function loadBoardData() {
            try {
                ...
                document.getElementById('progress-text').textContent =
                    `${progress.done} of ${progress.total} tasks completed (${progress.percentage}%)`;
            } catch (error) {
                console.error('Error loading board data:', error);
            }
        }
        loadBoardData();
        // drag/drop + task form logic would go here
    }
});
